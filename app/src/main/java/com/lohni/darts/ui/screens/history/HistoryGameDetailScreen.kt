...
@Serializable
data class HistoryGameDetailRoute(
    val gameId: Int
)

@Composable
fun HistoryGameDetailScreen(navController: NavController) {
    val args = navController.currentBackStackEntry?.toRoute<HistoryGameDetailRoute>()

    val gameId = args!!.gameId
    val historyViewModel: HistoryViewModel = viewModel(factory = HistoryViewModel.Factory)
    historyViewModel.setGameAndSetId(gameId)
        ...
        Column(modifier = Modifier.padding(start = 16.dp, top = 16.dp, end = 16.dp)) {
            Row(
                ...
                Column(
                    ...
                    Text("${game?.gameId ?: ""} - ${game?.gameMode ?: ""}", fontSize = 18.sp)
                }
            }
            Card(
                modifier = Modifier
                ...
                Column(
                    ...
                    Row(
                        ...
                        HistoryGameStatistics(gameId)
                    }
                }
            }
            val sets = remember { historyViewModel.setSummaries }
            LazyColumn(
                modifier = Modifier.padding(start = 16.dp, end = 16.dp)
            ) {
                items(sets) {
                    HistoryGameSetEntry(it) { s ->
...
@Composable
fun HistoryGameSetEntry(setSummary: ShortSummary, onClick: (ShortSummary) -> Unit) {
    Row(
        verticalAlignment = Alignment.CenterVertically,
        modifier = Modifier
            .padding(top = 16.dp, bottom = 8.dp)
            .height(48.dp)
            .clickable { onClick.invoke(setSummary) }
    ) {
        Column(Modifier.weight(4.0F), verticalArrangement = Arrangement.Center) {
            Text("${stringResource(R.string.set)} ${setSummary.ordinal + 1}", fontSize = 18.sp)
        }
        val avgFormat = FormatUtil.createDecimalFormat("0.0")
        Column(Modifier.weight(3.0F), verticalArrangement = Arrangement.Center) {
            Row(
                ...
                Column {
                    Text(setSummary.winner.pName, textAlign = TextAlign.End)
                    Text(
                        "${stringResource(R.string.average_short_append)} ${
                            avgFormat.format(
                                setSummary.winnerAvg
                            )
                        }",
